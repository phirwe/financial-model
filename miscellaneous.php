...
if (array_key_exists('options', $_GET) || array_key_exists('options', $_GET))
{
    foreach($_GET['options'] as $value){
       ...
       mysqli_query($conn, "DELETE FROM Miscellaneous WHERE member_name = '$s[0]' and plot_number = '$s[1]' and payment_date = '$s[2]' and total_amount = '$s[3]' and description = '$s[4]' and payment_method = '$s[5]' and cheque_number = '$s[6]'");
