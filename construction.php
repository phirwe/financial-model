...
if (array_key_exists('options', $_GET) || array_key_exists('options', $_GET))
{
    foreach($_GET['options'] as $value){
       ...
       mysqli_query($conn, "DELETE FROM Construction WHERE description = '$s[0]' and pay_to = '$s[1]' and payment_date = '$s[2]' and payment_method = '$s[3]' and  cheque_number = '$s[4]' and amount_spent = '$s[5]' and late_fee_fine = '$s[6]' and total_amount = '$s[7]'  ");
