...
if (array_key_exists('options', $_GET) || array_key_exists('options', $_GET))
{
    foreach($_GET['options'] as $value){
       ...
       mysqli_query($conn, "DELETE FROM ClubHouse WHERE member_name = '$s[0]' and plot_number = '$s[1]' and hours_rented = '$s[2]' and cost = '$s[3]' and date_rented = '$s[4]' and payment_date = '$s[5]' and payment_method = '$s[6]' and cheque_number = '$s[7]' ");
